...
<body class="bg-slate-100 text-slate-800 min-h-screen font-sans antialiased relative"
            ...
            @auth
                <div class="relative flex items-center space-x-3" x-data="{ open: false }">
                    ...
                    <button @click="open = !open" @click.outside="open = false" class="flex items-center space-x-3 focus:outline-none group">
                        <span class="font-medium text-slate-600 group-hover:text-blue-600 transition text-sm hidden sm:inline-block">
                            {{ auth()->user()->name }}
                        </span>
                        <div class="w-10 h-10 rounded-full overflow-hidden border-2 border-blue-500 group-hover:border-blue-600 transition bg-slate-200 flex items-center justify-center shadow-inner relative">
                        ...
                        <i class="fa-solid fa-chevron-down text-xs text-slate-400 group-hover:text-blue-600 transition pt-1"></i>
                    </button>
                    ...
                    <div x-show="open" 
                         ...
                         class="absolute right-0 top-12 mt-2 w-48 bg-white border border-slate-200 rounded-lg shadow-lg py-1 z-50"
                         style="display: none;">
                        
                        <div class="px-4 py-2 border-b border-slate-100 text-xs text-slate-400 uppercase font-semibold tracking-wider">
                            Account Options
                        </div>

                        <a href="{{ route('admin.logout') }}" class="flex items-center space-x-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 font-medium transition">
