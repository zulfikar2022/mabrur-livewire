    ...
    public function getPendingOrdersGroupedProperty()
    {
        ...
        if (!empty($this->userSearch)) {
            $query->whereHas('user', function ($userQuery) {
                // Using whereRaw to force lowercase comparison for true case-insensitivity
...
<div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-8">
        @forelse($this->pendingOrdersGrouped as $userId => $userOrders)
            ...
            @endphp
            <livewire:admin.order-list-item :user="$user" :user-orders="$userOrders" :status="OrderState::PENDING" :key="'user-order-'.$userId" />
        @empty
