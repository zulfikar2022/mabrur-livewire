...
new #[Layout('layouts.admin')] class extends Component {
    ...
    /**
     * Computed Property: Fetch and group cancelled orders with filters
     */
    public function getCancelledOrdersGroupedProperty()
    {
        $query = Order::whereHas('orderState', function ($query) {
            $query->where('name', OrderState::CANCELLED);
        })
...
<div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-8">
        @forelse($this->cancelledOrdersGrouped as $userId => $userOrders)
            @php 
                $user = $userOrders->first()->user; 
            @endphp
            <livewire:admin.order-list-item :user="$user" :user-orders="$userOrders" :status="OrderState::CANCELLED" :key="'cancelled-order-'.$userId" />
        @empty
            <div class="text-center py-12">
                <i class="fa-solid fa-box-open text-gray-300 text-6xl mb-4"></i>
                <p class="text-gray-500">No cancelled orders match your search.</p>
            </div>
        @endforelse
    </div>
</div>