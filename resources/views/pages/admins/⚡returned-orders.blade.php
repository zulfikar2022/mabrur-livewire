...
new #[Layout('layouts.admin')] class extends Component {
    ...
    /**
     * Computed Property: Fetch and group returned orders with filters
     */
    public function getReturnedOrdersGroupedProperty()
    {
        $query = Order::whereHas('orderState', function ($query) {
            $query->where('name', OrderState::RETURNED);
        })
...
<div>
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-black text-gray-800 tracking-tight flex items-center gap-3">
            <i class="fa-solid fa-undo text-red-500"></i>
            Returned Orders
        </h1>
    </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-8">
        @forelse($this->returnedOrdersGrouped as $userId => $userOrders)
            @php 
                $user = $userOrders->first()->user; 
            @endphp
            <livewire:admin.order-list-item :user="$user" :user-orders="$userOrders" :status="OrderState::RETURNED" :key="'returned-order-'.$userId" />
        @empty
            <div class="text-center py-12">
                <i class="fa-solid fa-box-open text-gray-300 text-6xl mb-4"></i>
                <p class="text-gray-500">No returned orders match your search.</p>
            </div>
        @endforelse
    </div>
</div>