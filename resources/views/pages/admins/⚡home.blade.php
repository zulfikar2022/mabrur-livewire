...
new #[Layout('layouts.admin')] class extends Component {
    public function getStatsProperty()
    {
        return [
            ...
            // Counts categories that have are  active (is_available is true)
            'active_categories' => Category::where('is_available', true)->count(),
...
<div class="p-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
        <a href="{{ route('admin.show-all-products') }}" wire:navigate class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <a href="{{ route('admin.see-all-users') }}" wire:navigate class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <a href="{{ route('admin.see-disabled-users') }}" wire:navigate class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        ...
        <a href="{{ route('admin.show-all-categories') }}" wire:navigate class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        @foreach(['pending', 'approved', 'shipped', 'delivered', 'deliver_failed', 'returned', 'cancelled'] as $state)
            <a href="{{ route('admin.' . $state . '-orders') }}" wire:navigate class="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                <p class="text-sm text-gray-500 uppercase font-semibold">{{ str_replace('_', ' ', $state) }}</p>
